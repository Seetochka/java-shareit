...
class ItemRequestServiceImplUnitTest {
    ...
    @Test
    void testGetAllItemRequest() throws InvalidParameterException, ObjectNotFountException {
    ...
    @Test
    void testGetAllItemRequestFailValidation() {
        Exception exception1 = assertThrows(InvalidParameterException.class, () ->
                itemRequestService.getAllItemRequest(mockUser1.getId(), -1, 20));

        assertEquals("Передан некорректный параметр from", exception1.getMessage());

        Exception exception2 = assertThrows(InvalidParameterException.class, () ->
                itemRequestService.getAllItemRequest(mockUser1.getId(), 0, -1));

        assertEquals("Передан некорректный параметр size", exception2.getMessage());
    }
